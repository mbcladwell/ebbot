...
(define %escape-chars
 ...
 (define (escape c)
   (let ((escaped (hash-ref %escape-chars c)))
     (if escaped escaped (string c))))

(define (recurse-escape in out)
  (if (null? (cdr in))
      (let* ((escaped (escape (car in)))
	    (new-s  (if escaped escaped (string (car in))))
	    (_ (set! out (string-append out new-s))))
	out)
      (let* ((escaped (escape (car in)))
	    (new-s  (if escaped escaped (string (car in))))
	    (_ (set! out (string-append out new-s))))
	(recurse-escape (cdr in) out))))


(define (string->escaped-html s)
  (recurse-escape (string->list s) ""))	
